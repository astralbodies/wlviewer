...
wss.on("connection", (ws) => {
  ...
  // Send cached weather data immediately if available
  if (cachedWeatherMessage) {
    console.log("Sending cached weather data to new client");
    ws.send(JSON.stringify(cachedWeatherMessage));
  }
...
function broadcastToClients(data) {
  // Cache the latest message for new connections
  cachedWeatherMessage = data;
...
function discoverWeatherLinkDevice() {
  ...
  setTimeout(() => {
    if (!weatherLinkDevice) {
      console.log(
        "No WeatherLink Live device found after 10 seconds, will retry in 30 seconds..."
      );
      browser.stop();
      bonjour.destroy();
            ...
            if (udpPort !== newPort) {
              ...
              udpSocket.on("message", (msg, rinfo) => {
                try {
                  ...
                  console.log(
                    `Received UDP data from ${rinfo.address}:${rinfo.port}`
                  );
    ...
    udpData.conditions.forEach((udpCondition, index) => {
      if (merged.conditions[index]) {
        if (udpCondition.wind_speed_last !== undefined) {
          merged.conditions[index].wind_speed_last =
            udpCondition.wind_speed_last;
        }
        ...
        if (udpCondition.rain_storm_start_at !== undefined) {
          merged.conditions[index].rain_storm_start_at =
            udpCondition.rain_storm_start_at;
        }
        ...
        if (udpCondition.rainfall_monthly !== undefined) {
          merged.conditions[index].rainfall_monthly =
            udpCondition.rainfall_monthly;
        }
